...
class FlowLoss(nn.Module):
    ...
    def forward(
        self, target, z, diffusion_batch_mul=4, pred_len=96
    ):
        ...
        loss = (weights * (predict_v - target) ** 2)[:, :pred_len].mean(dim=-1)

        loss = torch.stack(torch.chunk(loss, diffusion_batch_mul), dim=0).mean(dim=0)
        return loss
