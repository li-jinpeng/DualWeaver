...
class TimerForPrediction(TimerPreTrainedModel, TSGenerationMixin):
    ...
    def forward(
        ...
        if revin:
            mean, std = (
                input_ids.mean(dim=-1, keepdim=True).detach(),
                input_ids.std(dim=-1, keepdim=True).detach(),
            )
            std = torch.where(
                std > 1e-2, std, torch.tensor(1.0, device=input_ids.device)
            )
        ...
        if labels is not None:
            ...
            for lm_head, output_token_len in zip(
                ...
                one_loss = self.calc_ar_loss(
                    one_predictions, labels, output_token_len
                )
                ar_loss += one_loss
    ...
    def calc_ar_loss(self, predictions, labels, output_token_len):
